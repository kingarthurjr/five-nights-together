...
function udphp_serverPunch() {
	...
	*/
	...
	for(i=0; i<ds_map_size(incoming_requests); i+=1) {
	    var delete_ = false; //flag that get's set to true when this entry should be removed from incoming_requests
	    ...
	    if (delete_) ds_map_delete(incoming_requests,key);
	    key = tmpkey;
	}
	...
	for(i=0; i<ds_map_size(incoming_requests2); i+=1) {
	    var delete_ = false; //flag that get's set to true when this entry should be removed from incoming_requests2
	    ...
	    if (delete_) ds_map_delete(incoming_requests2,key);
	    key = tmpkey;
	}

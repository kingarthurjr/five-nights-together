...
function htme_init() {
	...
	*/
	...
	if steam_enabled
	{
	    if !instance_exists(obj_steam_) instance_create(0,0,obj_steam_);
	}
	else
	{
	    with obj_steam_ instance_destroy();
	}
	...
	if (self.use_udphp) {
	    ...
	    script_execute(asset_get_index("udphp_config"),self.udphp_master_ip, self.udphp_master_port,self.udphp_rctintv,self.global_timeout,u_debug,u_silent,self.use_delta_time,self.upnp_enabled);

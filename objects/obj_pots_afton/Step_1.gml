...
{
    if(!place_meeting(x, y - spd, obj_minigame_border)) if ((time % 2) == 0){ y -= spd; audio_play_sound(snd_run,10,false);}
    step++;
}
...
{
    if(!place_meeting(x - spd, y, obj_minigame_border)) if ((time % 2) == 0){ x -= spd; audio_play_sound(snd_run,10,false);}
    step++;
}
